...
function Protected({ children, roles }) {
  ...
  if (loading) return <div className="p-8 text-center text-ink-500">Cargando...</div>;
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
          <div className="col-span-2 md:col-span-1">
            <div className="font-extrabold text-xl mb-1">Rentar</div>
        ...
        <div className="pt-6 border-t border-white/10 flex flex-col sm:flex-row justify-between gap-2 text-[11px] text-white/40">
...
function FooterCol({ title, links }) {
  return (
    <div>
      <div className="text-[10px] font-bold uppercase tracking-widest text-white/40 mb-3">{title}</div>
      {links.map(l => (
        <a key={l} href="#" className="block text-[12px] text-white/65 hover:text-white mb-1.5 transition">{l}</a>
      ))}
    </div>
  );
}

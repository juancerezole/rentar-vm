  ...
  return (
    <div className="bg-white rounded-2xl border border-ink-200 p-5 shadow-soft space-y-5">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-ink-900 text-[15px]">Filtros</h3>
        <button
          onClick={onClear}
          className="text-xs inline-flex items-center gap-1 text-ink-400 hover:text-ink-700 transition"
        >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-ink-200 text-sm focus:border-brand-mid focus:ring-2 focus:ring-brand-soft outline-none transition bg-white"
        />
      </div>
            ...
            <button
              ...
              className={`text-xs font-medium rounded-full border px-2.5 py-1 transition
                ${filters.tipo === t.v
                  ...
                  : 'bg-white text-ink-500 border-ink-200 hover:border-brand-border hover:text-ink-900'
                }`}
            >
              {t.label}
            </button>
          ...
          {AMBIENTES.map(a => (
            <button
              ...
              className={`py-1.5 text-sm font-semibold rounded-lg border transition
                ${filters.ambientes === a
                  ? 'bg-brand text-white border-brand shadow-soft'
                  : 'bg-white text-ink-500 border-ink-200 hover:border-brand-border'
                }`}
            >
              {a}
            </button>
      ...
      <Group label="Precio mensual">
        <div className="grid grid-cols-2 gap-2">
          <input
            ...
            onChange={e => set('minPrecio', e.target.value)}
            className={priceCls}
          />
          <input
            type="number"
            ...
            onChange={e => set('maxPrecio', e.target.value)}
            className={priceCls}
          />
        </div>
      </Group>
        ...
        <div className="space-y-1">
          {[
            ...
            return (
              ...
              >
                <span className={`w-3.5 h-3.5 rounded-full border-2 shrink-0 ${sel ? 'border-brand bg-brand' : 'border-ink-300'}`} />
                {g.l}
              </button>
            );
          })}
...
const priceCls = 'w-full px-3 py-2 rounded-xl border border-ink-200 text-sm focus:border-brand-mid focus:ring-2 focus:ring-brand-soft outline-none bg-white text-center transition';
const selectCls = 'w-full px-3 py-2 rounded-xl border border-ink-200 text-sm focus:border-brand-mid focus:ring-2 focus:ring-brand-soft outline-none bg-white transition';
...
function Toggle({ label, sub, value, onChange }) {
  return (
    ...
    >
      ...
      <span className={`w-9 h-5 rounded-full relative transition-colors ${value ? 'bg-brand-mid' : 'bg-ink-200'}`}>

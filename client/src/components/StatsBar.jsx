...
export default function StatsBar() {
  ...
  return (
    <div className="bg-white border-b border-ink-200">
      <div className="max-w-7xl mx-auto px-4 h-10 flex items-center text-[12px] divide-x divide-ink-200 gap-0">
        <Item label="Dólar blue" value={`$${data.dolarBlue.compra} / $${data.dolarBlue.venta}`} extra={<span className="text-emerald-600 inline-flex items-center gap-0.5"><TrendingUp className="w-3 h-3" />{data.dolarBlue.variacion}</span>} accent />
        ...
        {data.liquidacion > 0 && (
          <div className="ml-auto pl-4 flex items-center gap-1.5">
            <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-accent-orange bg-accent-orange-soft border border-accent-orange-border rounded-full px-2.5 py-0.5">
              <Flame className="w-3 h-3" /> {data.liquidacion} en liquidación hoy
            </span>
          </div>
        )}
      </div>

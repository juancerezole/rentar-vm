  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-7">
        <div>
          <h1 className="text-2xl font-bold text-ink-900 flex items-center gap-2">
            <Building2 className="w-6 h-6 text-brand-mid" />
            Mis propiedades
          </h1>
        ...
        <button
          onClick={startCreate}
          className="inline-flex items-center gap-2 bg-brand hover:bg-brand-dark text-white font-semibold px-4 py-2.5 rounded-xl shadow-soft transition"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {list.length === 0 && (
          <div className="col-span-full bg-white rounded-2xl border-2 border-dashed border-ink-200 p-14 text-center">
            <Building2 className="w-10 h-10 text-ink-300 mx-auto mb-3" />
            <p className="text-ink-700 font-medium">Todavía no cargaste propiedades</p>
            <p className="text-sm text-ink-400 mt-1">Hacé clic en "Nueva propiedad" para empezar.</p>
          </div>
        )}
        {list.map(p => (
          <div key={p.id} className="bg-white rounded-2xl border border-ink-200 shadow-soft hover:shadow-card hover:border-brand-border transition-all overflow-hidden flex flex-col">
            ...
            <div className="p-3.5 flex-1 flex flex-col">
              <div className="text-[10px] uppercase tracking-widest text-ink-400 font-semibold">{tipoLabel(p.tipo)}</div>
              <h3 className="font-semibold text-sm text-ink-900 line-clamp-1 mt-0.5">{p.titulo}</h3>
              <div className="text-xs text-ink-400 line-clamp-1 mt-0.5">{p.direccion} · {p.barrio}</div>
              <div className="font-bold text-brand mt-2">{formatPrice(p.precio)}</div>
              <div className="flex gap-1.5 mt-3 pt-3 border-t border-ink-100">
                <button
                  onClick={() => startEdit(p)}
                  className="flex-1 inline-flex items-center justify-center gap-1.5 text-xs font-medium text-ink-600 hover:bg-ink-100 rounded-lg py-1.5 transition"
                >
                  <Pencil className="w-3.5 h-3.5" /> Editar
                </button>
                <button
                  onClick={() => onDelete(p.id)}
                  className="flex-1 inline-flex items-center justify-center gap-1.5 text-xs font-medium text-red-500 hover:bg-red-50 rounded-lg py-1.5 transition"
                >
      ...
      {open && (
        <div
          className="fixed inset-0 z-50 flex items-start md:items-center justify-center p-4 bg-ink-900/50 backdrop-blur-sm"
          onClick={() => setOpen(false)}
        >
          <div
            onClick={e => e.stopPropagation()}
            className="bg-white rounded-2xl shadow-lg max-w-3xl w-full max-h-[92vh] overflow-y-auto scrollbar-thin animate-fadeIn"
          >
            <div className="sticky top-0 bg-white border-b border-ink-100 px-6 py-4 flex items-center justify-between">
              ...
              <button onClick={() => setOpen(false)} className="p-1.5 rounded-lg hover:bg-ink-100 transition">
            ...
            <form onSubmit={onSubmit} className="p-6 space-y-5">
              <Field label="Título de la publicación *">
                <input
                  ...
                  required
                  placeholder="Ej: Depto luminoso 2 amb. en pleno centro"
                  className={inputCls}
                />
              </Field>
              ...
              <Field label="Dirección *">
                <input
                  ...
                  required
                  placeholder="Ej: Av. Sabattini 1234"
                  className={inputCls}
                />
              </Field>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`py-2 rounded-xl text-sm font-medium border transition ${
                        form.garantia === g.v
                          ? 'bg-brand-soft border-brand-mid text-brand'
                          : 'border-ink-200 text-ink-600 hover:bg-ink-100/50'
                      }`}
                    >
                      {g.l}
                    </button>
              ...
              <Field label="Descripción">
                <textarea
                  ...
                  className={inputCls}
                  placeholder="Detalles, comodidades, ubicación..."
                />
              </Field>
              ...
              <Field label="URL de imagen principal">
                ...
                <div className="flex gap-2 mt-2 overflow-x-auto scrollbar-thin pb-1">
                  {SAMPLE_IMAGES.map(src => (
                    <button
                      key={src}
                      type="button"
                      onClick={() => setForm({ ...form, imagen: src })}
                      className={`w-20 h-14 shrink-0 rounded-xl overflow-hidden border-2 transition ${
                        form.imagen === src ? 'border-brand-mid shadow-soft' : 'border-transparent hover:border-ink-300'
                      }`}
                    >
                ...
                <p className="text-[11px] text-ink-400 mt-1">Tip: elegí una imagen de muestra o pegá una URL.</p>
              </Field>
              ...
              <div className="flex gap-2 pt-4 border-t border-ink-100">
                <button
                  type="button"
                  onClick={() => setOpen(false)}
                  className="flex-1 py-2.5 rounded-xl border border-ink-200 font-semibold text-ink-600 hover:bg-ink-100 transition"
                >
                  Cancelar
                </button>
                <button
                  disabled={saving}
                  className="flex-1 py-2.5 rounded-xl bg-brand hover:bg-brand-dark disabled:opacity-60 text-white font-semibold transition shadow-soft"
                >
...
const inputCls = 'w-full px-3 py-2.5 rounded-xl border border-ink-200 text-sm focus:ring-2 focus:ring-brand-soft focus:border-brand-mid outline-none bg-white transition';
  ...
  return (
    <button
      ...
      className={`flex items-center justify-between text-sm px-3 py-2.5 rounded-xl border transition ${
        checked
          ? 'bg-success-soft border-success-border text-success-dark'
          : 'bg-white border-ink-200 text-ink-600 hover:bg-ink-100/50'
      }`}
    >
      <span>{label}</span>
      <span className={`w-5 h-5 rounded-lg flex items-center justify-center text-xs font-bold transition ${
        checked ? 'bg-success text-white' : 'bg-ink-100'
      }`}>
        {checked ? '✓' : ''}
      </span>
    </button>
